...
deckerShakeOptions ctx = do
    ...
    return $
        shakeOptions
            { shakeFiles = ".shake"
            , shakeExtra =
                  HashMap.insert actionContextKey (toDyn ctx) HashMap.empty
            , shakeThreads = cores
            , shakeAbbreviations =
                  [ (ctx ^. dirs . project ++ "/", "")
                  , (ctx ^. dirs . public ++ "/", "")
...
actionContext = do
    ...
    return $
        fromMaybe (error "Error upcasting action context") (fromDynamic dyn)
...
openBrowser url =
    if | any (`isInfixOf` os) ["linux", "bsd"] ->
       ...
       | otherwise ->
           putNormal $ "Unable to open browser on this platform for url: " ++
           url
