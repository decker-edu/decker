...
blockToMarkdown' opts t@(Table caption aligns widths headers rows) =  do
  ...
  let widths' = case numcols - length widths of
                     x | x > 0 -> widths ++ replicate x 0.0
                       | otherwise -> widths
  let nowrap = opts{writerWrapText=WrapNone}
  (nst,tbl) <-
     case True of
          _ | isSimple &&
              isEnabled Ext_simple_tables nowrap -> do
                rawHeaders <- padRow <$> mapM (blockListToMarkdown nowrap) headers
                rawRows <- mapM (fmap padRow . mapM (blockListToMarkdown nowrap))
                           rows
                (nest 2,) <$> pandocTable nowrap False (all null headers)
                                aligns' widths' rawHeaders rawRows
            | isSimple &&
              isEnabled Ext_pipe_tables nowrap -> do
                rawHeaders <- padRow <$> mapM (blockListToMarkdown nowrap) headers
                rawRows <- mapM (fmap padRow . mapM (blockListToMarkdown nowrap))
                           rows
                (id,) <$> pipeTable (all null headers) aligns' rawHeaders rawRows
            | not hasBlocks &&
              isEnabled Ext_multiline_tables nowrap -> do
                rawHeaders <- padRow <$> mapM (blockListToMarkdown nowrap) headers
                rawRows <- mapM (fmap padRow . mapM (blockListToMarkdown nowrap))
                           rows
                (nest 2,) <$> pandocTable nowrap True (all null headers)
                                aligns' widths' rawHeaders rawRows
            | isEnabled Ext_grid_tables nowrap &&
               writerColumns nowrap >= 8 * numcols -> (id,) <$>
                gridTable nowrap blockListToMarkdown
