...
import Text.Decker.Writer.Html (writeNativeWhileDebugging)
...
markdownToHtml disp meta getTemplate markdownFile out = do
  ...
  writePandocFile options out (addSourcePath markdownFile pandoc)

-- writeNativeWhileDebugging out "wat" pandoc

addSourcePath path (Pandoc meta blocks) =
  Pandoc (setMetaValue "map-source.path" path meta) blocks
