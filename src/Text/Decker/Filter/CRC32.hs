...
-}
module Text.Decker.Filter.CRC32 where
...
update_crc crc ch =
  let c = crc `xor` 0xFFFFFFFF
      newval =
        (gzipcrctab ! fromIntegral ((c `xor` fromIntegral (ord ch)) .&. 0xff)) `xor`
        (c `shiftR` 8)
   in newval `xor` 0xFFFFFFFF

update_crc_list :: Word32 -> [Char] -> Word32
update_crc_list = foldl update_crc

calc_crc32 :: [Char] -> Word32
calc_crc32 = update_crc_list 0
