...
quizMap :: Map Text.Text Text.Text
quizMap = Map.fromList
  [ ("quiz-match-items"    , Text.pack $ show Match)
  , ("quiz-mi"             , Text.pack $ show Match)
  , ("qmi"                 , Text.pack $ show Match)
  , ("quiz-multiple-choice", Text.pack $ show Mult)
  , ("quiz-mc"             , Text.pack $ show Mult)
  , ("qmc"                 , Text.pack $ show Mult)
  , ("quiz-insert-choices" , Text.pack $ show Ins)
  , ("quiz-ic"             , Text.pack $ show Ins)
  , ("qic"                 , Text.pack $ show Ins)
  , ("quiz-free-text"      , Text.pack $ show Free)
  , ("quiz-ft"             , Text.pack $ show Free)
  , ("qft"                 , Text.pack $ show Free)
  ]


data Quiz = Match | Mult | Ins | Free deriving Show
...
renderFreetextQuestion bl@(BulletList ((firstBlock : _) : (sndBlock : _) : _))
  = case
      (checkIfFreetextQuestion firstBlock, checkIfFreetextAnswer sndBlock)
    of
      (Just q, Just a) -> Div
        ("", ["freetextQuestion"], [])
...
blanktextHtml (inlines, blocks) = Div ("", ["blankText"], [])
                                      ([title] ++ selects ++ [answerButton])
 where
  ...
  splitBlankText inlines = concatMap
    (split (startsWith "{"))
...
blanktextHtmlAnswers = concatMap
  (\x ->
    ...
    then
      [ U.toHtml
          (Text.pack
            (printf "<input type=\"text\" answer=\"%s\" class=\"blankInput\">"
         ...
         if "{" `isPrefixOf` x && "}" `isSuffixOf` x && "|" `isInfixOf` x
      then
        [U.toHtml "<select class=\"blankSelect\">"]
...
matchingHtml dListItems = Div ("", ["matching"], [])
                              [dropzones, dragzone, answerButton]
 where
  ...
  wrapDrop inlines = Div ("", ["dropzones"], []) dropzones
    where dropzones = (\i -> Div ("", ["dropzone"], []) [Plain i]) <$> inlines
