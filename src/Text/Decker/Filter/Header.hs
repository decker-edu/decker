...
transformHeader1 h1 @ (Header 1 headAttr inlines)
  ...
  where
      ...
      = do uri <- transformUrl url ""
           runAttrOn headAttr imgAttr
             $ case classifyMedia uri imgAttr of
               ImageT
                -> do injectAttribute
                        ( "data-background-image", URI.render uri )
                      passAttribs
                        ("data-background-" <>)
               ...
               VideoT
                -> do injectAttribute
                        ( "data-background-video", URI.render uri )
                      takeClasses
               ...
               IframeT
                -> do injectAttribute
                        ( "data-background-iframe", URI.render uri )
                      takeClasses
                        ("data-background-" <>)
                        [ "interactive" ]
                      passAttribs
    ...
    buildHeader _
      = bug
      $ InternalException "transformHeader: no last image in header"
...
lastImage inlines
  = ( fromJust $ listToMaybe $ reverse $ query image inlines
    , zapImages inlines
    )
  where
    image i @ (Image {}) = [ i ]
