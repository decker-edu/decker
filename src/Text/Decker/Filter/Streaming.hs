...
streamHtml uri caption =
    do let scheme = uriScheme uri
       width <- srcAttribute "width"
       height <- srcAttribute "height"
       let args =
               maybe
                   []
                   (\w -> maybe [ w ] (\h -> [ w, h ]) height)
                   width
       attr <- src
       streamId <- case URI.uriAuthority uri of
...
streamHtml' uri caption =
    do let scheme = uriScheme uri
       ...
       case caption of
           [] -> do divAttr <- injectClass "nofigure" >>
                        injectBorder >>
                        takeSize >>
                        takeUsual >>
                        extractAttr
...
takeWrapperAttr =
    do aspect <- calcAspect . fromMaybe "" <$> cutAttrib "aspect"
...
takeIframeAttr =
    do injectStyle ( "position", "absolute" )
       injectStyle ( "top", "0" )
       injectStyle ( "left", "0" )
...
calcAspect ratio =
    ...
    case Text.splitOn ":" ratio of
        [ w, h ] ->
            do wf <- readMaybe $ toString w :: Maybe Float
...
mkMediaTag tag uri dataSrc attr =
    let srcAttr = if dataSrc then H.dataAttribute "src" else A.src
        in mkAttrTag tag attr !
           srcAttr (H.preEscapedToValue $ URI.render uri)

mkStreamTag :: URI -> Attr -> Attr -> Html
mkStreamTag uri wrapperAttr iframeAttr =
    let inner =
            mkMediaTag
                (H.iframe "Iframe showing video here.")
                uri
                False
                iframeAttr
