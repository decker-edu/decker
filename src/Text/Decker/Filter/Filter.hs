...
processPandoc :: (Pandoc -> Decker Pandoc)
              -> FilePath
              -> Disposition
              -> Provisioning
              -> Pandoc
...
splitJoinColumns slide @ (Slide header body) =
  do disp <- gets disposition
     case disp of
       Disposition Deck Html
         -> return $ Slide header $ concatMap wrapRow rowBlocks
         where rowBlocks =
                 split (keepDelimsL $ whenElt (hasAnyClass ["split","join"]))
                       body
...
handleBackground slide @ (Slide header blocks) =
  case header of
    Just (Header 1 (headerId,headerClasses,headerAttributes) inlines)
      -> case query allImages inlines of
        image @ (Image (_,imageClasses,imageAttributes) _ (imageSrc,_)):_
          -> do disp <- gets disposition
                case disp of
                  Disposition Deck Html
                    -> do let cls = headerClasses ++ imageClasses
                          return
                            $ Slide (Just (Header -- Construct a new header with the necessary attributes for RevealJs background content
                                           1
                                           (headerId
                                           ,cls
  ...
  where transform ("size",value) = ("data-background-size",value)
...
wrapBoxes slide @ (Slide header body) =
  do disp <- gets disposition
     case disp of
...
mapSlides action (Pandoc meta blocks) =
  do slides <- selectActiveContent (toSlides blocks)
...
filterNotebookSlides (Pandoc meta blocks) =
  let inNotebook = fromSlides $ filter notebook (toSlides blocks)
      ...
      strip block = block
      notebook slide =
        "notebook" `elem` (view (attributes . attrClasses) slide)
...
deDiv :: [Block] -> [Block]
deDiv = foldr flatten []
  where flatten (Div attr blocks) result = blocks ++ result
...
processSlides pandoc = mapSlides (concatM actions) pandoc
  where actions :: [Slide -> Decker Slide]
        actions =
...
useCachedImages cacheDir
                image @ (Image (ident,cls,values) inlines (url,imgTitle)) =
...
renderMediaTag disp (Image attrs @ (ident,cls,values) [] (url,tit)) =
  ...
  where imageVideoTag =
          if uriPathExtension url == ".svg" && "embed" `elem` cls
             then do fileContent <- catch (readFile url) svgLoadErrorHandler
                     return
                       $ if attrs /= nullAttr
        ...
        createMediaTag =
          case classifyFilePath url cls of
            ...
            IframeMedia
              -> mediaTag (H.iframe "Browser does not support iframe.")
        ...
        transformedValues =
          case classifyFilePath url cls of
            VideoMedia
              -> lazyLoad $ retrieveVideoStart $ transformImageSize values
...
renderMediaTag disp (Image (ident,cls,values) inlines (url,tit)) =
  do image <- renderMediaTag disp (Image attrsForward [] (url,tit))
     return
       $ Span nullAttr
              ([toHtml "<figure>",image,toHtml "<figcaption>"]
               ++ inlines
               ++ [toHtml "</figcaption>",toHtml "</figure>"])
  where attrsForward = (ident,cls,("alt",stringify inlines):values)
...
convertMediaAttributeGatherStyle (id,cls,vals) = (id,cls,vals')
  where (style_cls,cls') = partition (\x -> fst x == "style") vals
...
convertMediaAttributeImageSize (id,cls,vals) = (id,cls,vals_processed)
  where (height,vals') = partition (\x -> fst x == "height") vals
...
transformImageSize attributes =
  let style :: [String]
      style =
        ...
        $ fromMaybe ""
        $ snd <$> find (\(k,_) -> k == "style") attributes
...
retrieveVideoStart attributes = (attributeRest,urlStartMarker)
  where attributeRest = filter (\(k,_) -> k /= "start") attributes
