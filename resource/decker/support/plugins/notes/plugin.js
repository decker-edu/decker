// Needs to be converted into a fetch
// import speakerViewHTML from "./speaker-view.html";

let speakerViewHTML;

async function loadSpeakerViewHTML() {
  const url = new URL(import.meta.url);
  const path = url.pathname.substring(0, url.pathname.lastIndexOf("/"));
  const response = await fetch(path + "/speaker-view.html");
  if (response && response.ok) {
    speakerViewHTML = await response.text();
  } else {
    speakerViewHTML =
      "<html><head></head><body><h1>Unable to load the speaker view webpage</h1></body></html>";
  }
}

// Decker does not use the markdown feature of reveal
