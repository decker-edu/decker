...
const MarioQuiz = (function () {
  ...
  // get config
  let config = Reveal.getConfig().quiz || {};

  // get quiz server
  let server = config.server || "http://graphics.uni-bielefeld.de:8080";
  ...
  function scriptPath() {
    ...
    if (document.currentScript) {
      ...
      let sel = document.querySelector('script[src$="/quiz.js"]');
      if (sel) {
  ...
  qrscript.onload = function () {
    ...
    Array.from(document.getElementsByClassName("quiz-qr")).forEach(function (
      e
    ) {
  ...
  function drawChart(result) {
    ...
    let labels = ["A", "B", "C", "D", "E", "F", "G", "H"].slice(
      0,
      result.length
    );
...
Reveal.registerPlugin("quiz", MarioQuiz);
